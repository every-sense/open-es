module DataAbstraction::SensorData
  class Location < Generic
    ...
    def initialize(data, meta_values = {})
      ...
      @dimansion_unit = data['dimension_unit'] ? data['dimension_unit']  : "m"
      if  ( data['values'] )
        @values = Array.new
        if ( data['values'].instance_of? Array )
          ...
          @values[2] = DimensionValue.new(data['values'][2].to_f, @dimansion_unit)
          ...
          @values[2] = DimensionValue.new(data['values']['elevation'].to_f, @dimansion_unit)
        end
      else
